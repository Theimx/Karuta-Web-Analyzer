...
# Charger l'image
input_image_path = 'AutoSketch/dio-brando-7.jpg'  # Remplacez par le chemin de votre image
output_image_path = 'AutoSketch/dio-brando-72.jpg'  # Chemin pour sauvegarder l'image modifiée
image = Image.open(input_image_path).convert('RGB')
pixels = np.array(image, dtype=np.uint8)  # Assurer un format uint8

# Reshape pour comparer en une seule opération vectorisée
pixels_flat = pixels.reshape(-1, 3)

# Calcul de la distance euclidienne avec numpy.linalg.norm
def find_closest_color(pixel):
    distances = np.linalg.norm(available_colors - pixel, axis=1)
    return available_colors[np.argmin(distances)]

# Appliquer la correspondance des couleurs
new_pixels_flat = np.apply_along_axis(find_closest_color, 1, pixels_flat)

# Reshape vers la forme d'origine
